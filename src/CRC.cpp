...
namespace shaga::CRC {
	...
	const uint_fast16_t _crc16_ccitt_false_table[256] = {
...
namespace shaga {
	...
	uint16_t CRC::crc16_ccitt_false (const void *const buf, const size_t len, const uint16_t startval)
	{
		uint_fast16_t crc = startval;
		const uint8_t *const pbuf = static_cast<const uint8_t*> (buf);

		for (uint_fast32_t pos {0}; pos < len; ++pos) {
			crc = (crc << 8) ^ _crc16_ccitt_false_table[static_cast<uint8_t> (((crc & UINT16_MAX) >> 8) ^ pbuf[pos])];
		}

		return crc & UINT16_MAX;
	}

	size_t CRC::crc16_ccitt_false_check (const std::string_view plain, const uint16_t startval)
	{
		const size_t sze = plain.size ();
		if (sze < sizeof (uint16_t)) {
			cThrow ("CRC-16: String is too short"sv);
		}
		const uint16_t crc = crc16_ccitt_false (plain.data (), sze - sizeof (uint16_t), startval);
		const uint16_t crc_check = BIN::to_uint16 (plain.data () + (sze - sizeof (uint16_t)));
		if (crc_check != crc) {
			cThrow ("CRC-16: Signature does not match"sv);
		}
		return (sze - sizeof (uint16_t));
	...
	void CRC::crc16_ccitt_false_check_and_trim (std::string_view &plain, const uint16_t startval)
	{
		const size_t sze = plain.size ();
		if (sze < sizeof (uint16_t)) {
			cThrow ("CRC-16: String is too short"sv);
	...
	void CRC::crc16_ccitt_false_append (std::string &plain, const uint16_t startval)
	{
		const uint16_t crc = crc16_ccitt_false (plain, startval);
		BIN::from_uint16 (crc, plain);
	}
